...
# -*- coding: utf-8 -*-
"""
ExtraTrees (Entropy) baseline + Coarse Grid Search (max_depth × min_samples_leaf)
- Automatically finds the best combination by OOF AUC
- Retrains with best params and exports: submission / OOF / feature importance /
  run config / grid results / best params
"""
...
CFG = {
    ...
    # You can also switch to your screened F.L.I feature files if desired
    "train_path": "../Features/All_application_train.csv",
    "test_path": "../Features/All_application_test.csv",

    "out_dir": "baseline_extratrees_entropy_grid",

    # ExtraTrees base params (grid will override max_depth & min_samples_leaf)
    "et_params": {
        "n_estimators": 800,          # e.g., 500–1200
        "criterion": "entropy",       # use "gini" for Gini impurity if desired
        "max_depth": None,            # overridden by grid
        "min_samples_split": 2,
        "min_samples_leaf": 1,        # overridden by grid
        "max_features": "sqrt",       # can try 0.5 / 0.8 / "log2"
        "bootstrap": False,           # ExtraTrees typically without bootstrapping
        "n_jobs": -1,
        "random_state": 42,
        "class_weight": "balanced",   # recommended for imbalanced data
    },
...
def preprocess(train, test, target, id_col):
    """Basic preprocessing: categorical encoding + missing value imputation."""
    ...
    # Label Encoding (fit on the combined data to avoid unseen categories)
    for col in cat_cols:
        le = LabelEncoder()
...
def run_cv_extratrees(train_pre, y, test_pre, features, cfg):
    """5-fold CV training of ExtraTrees; returns OOF, test predictions, OOF AUC, and feature importance."""
...
def grid_search_extratrees(train_pre, y, test_pre, features, base_cfg,
                           depths, min_leaf_list, save_csv_path=None):
    """
    Search only over: max_depth × min_samples_leaf.
    All other parameters follow base_cfg['et_params'].
    """
    ...
    for md, mleaf in iterator:
        ...
        cfg["et_params"].update({
            "max_depth": md,
            "min_samples_leaf": mleaf,
        })

        print(f"\n[Try] max_depth={md}, min_samples_leaf={mleaf}")
        oof, test_pred, oof_auc, fi = run_cv_extratrees(
        ...
        if oof_auc > best_auc:
            ...
            best_pack = {
                ...
                "min_samples_leaf": mleaf,
                "oof_auc": oof_auc,
                "detail": (oof, test_pred, fi, cfg)  # aligned with RF/LGB versions
            }
...
def main():
    ...
    print("Preprocessing (categorical encoding and missing value imputation)...")
    ...
    best_pack, tried_df = grid_search_extratrees(
        train_pre, y, test_pre, features, CFG,
    ...
    print("\n[Retrain] Re-training with best params and exporting files...")
    oof, test_pred, oof_auc, fi = run_cv_extratrees(

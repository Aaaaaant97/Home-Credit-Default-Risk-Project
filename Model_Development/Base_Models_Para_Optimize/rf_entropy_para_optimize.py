...
# -*- coding: utf-8 -*-
"""
RandomForest (Entropy) baseline + coarse grid search (max_depth × min_samples_leaf)
- Automatically finds the best combination (by OOF AUC)
- Retrains with the best parameters and exports submission / OOF / feature importance / run config
"""
...
CFG = {
    ...
    "target": "TARGET",
    "id_col": "SK_ID_CURR",
    # If you want to use filtered F.L.I features, switch to the following:
    ...
    "out_dir": "baseline_rf_entropy_grid",

    # RF base parameters (grid will override max_depth & min_samples_leaf; others stay fixed)
    "rf_params": {
        "n_estimators": 800,          # prioritize stability; reduce to ~500 for speed
        "criterion": "entropy",
        "max_depth": None,            # will be overridden by grid
        "min_samples_split": 2,
        "min_samples_leaf": 1,        # will be overridden by grid
        "max_features": "sqrt",
        "bootstrap": True,
        "n_jobs": -1,
        "random_state": 42,
...
def grid_search_rf_complexity(train_pre, y, test_pre, features, base_cfg,
    ...
    """
    Grid search over two complexity parameters: max_depth × min_samples_leaf.
    Other parameters follow base_cfg['rf_params'].
    """
    pairs = list(product(depths, min_leaf_list))
    records = []

    iterator = tqdm(pairs, desc="Grid search (max_depth × min_samples_leaf)") if _use_tqdm else pairs
...
def main():
    ...
    # Save original run config + best parameters
